...
// Fase Motorista-Pedagios — pedido do Daniel: "no PWA Motorista, em
// Roteirização, apresentar no mapa as praças de pedágio que estão na rota
// traçada" + "no resumo de custo total, colocar o valor total de pedágio
// também". Porta fiel de pedagio_service.dart (PWA Cliente) — reaproveita
// os mesmos helpers de bounding box/posição na rota já usados acima pros
// postos de combustível (_distanciasAcumuladas, _construirBoundingBoxes,
// _posicaoNaRotaKm), já que aqui não há carteira de postos própria e a
// tabela pracas_pedagio é de leitura pública pra qualquer autenticado.
class PracaPedagio {
  final int id;
  final String nome;
...
// — mesmo racional de CategoriaVeiculoPedagio no PWA Cliente/painel web.
enum CategoriaVeiculoPedagio { carro, moto, caminhao }

const _raioCorredorPedagioKm = 3.0;

Future<List<PracaPedagioNaRota>> buscarPracasPedagioNaRota(
  List<PontoRota> rota, {
  double raioKm = _raioCorredorPedagioKm,
}) async {
  if (rota.isEmpty) return [];
  final acumuladas = _distanciasAcumuladas(rota);
  final margemGraus = raioKm / 100;
  final boxes = _construirBoundingBoxes(rota, acumuladas, margemGraus);
  if (boxes.isEmpty) return [];
