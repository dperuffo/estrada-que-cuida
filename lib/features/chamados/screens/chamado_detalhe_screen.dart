...
class _ChamadoDetalheScreenState extends ConsumerState<ChamadoDetalheScreen> {
  ...
  Future<void> _enviarComentario(String telefone) async {
    ...
    try {
      await ChamadosService().comentar(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Não consegui enviar a mensagem.')),
        );
      }
    } finally {
      ...
      body: detalheAsync.when(
        ...
        data: (detalhe) {
          ...
          if (!_jaMarcouVisto) {
            _jaMarcouVisto = true;
            Future.microtask(
              () => ChamadosService().marcarVisto(widget.ticketId),
            );
          }
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    Text(
                      '${tiposTicket[t.tipo] ?? t.tipo} • ${statusTicket[t.status] ?? t.status} • Prioridade ${prioridadesTicket[t.prioridade] ?? t.prioridade}',
                    ),
                    const SizedBox(height: 16),
                    Text(t.descricao),
                    if (t.respostaAdmin != null &&
                        t.respostaAdmin!.trim().isNotEmpty) ...[
                      const SizedBox(height: 16),
                      Container(
                    ...
                    children: [
                      ...
                      IconButton.filled(
                        onPressed: _enviando
                            ? null
                            : () => _enviarComentario(
                                perfilAsync.valueOrNull?.telefone ?? '',
                              ),
                        icon: _enviando
                            ? const SizedBox(
                                height: 16,
